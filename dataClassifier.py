...
def enhancedFeatureExtractorDigit(datum):
    ...
    features['aspect_ratio'] = aspectRatio
    return features
...
def enhancedPacmanFeatures(state, action):
    ...
    features['stop_agent'] = state.isLose() | state.isWin()

    food = state.getFood().asList()
    if food:
        closestFood = min(map(lambda x: util.manhattanDistance(position, x), food))
        features['food_agent'] = 1.0 / (30 * closestFood)

    ghosts = state.getGhostPositions()
    if ghosts:
        closestGhost = min(map(lambda x: util.manhattanDistance(position, x), ghosts))
        features['suicide_agent'] = 1.0 / (10 * closestGhost)
